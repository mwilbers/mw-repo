...
/**
 * Test for basic database-functionality like create, read, update, delete (CRUD). Furthermore db-sequence-generator ist
 * tested here, some filter- and sorting-functionality and default-handling
 *
 * @author mwilbers
 *
 */
...
public class CRUDTest extends AbstractOfdbInitializationTest {
	...
	// 5. dann weiter mit Umbau ofdbCrudInterceptor und die chain verlegen nach ofdbService
	...
	public void testSequenceIncrementationAndInterceptor() throws OfdbMissingMappingException {
		...
		// You need to use Hibernate 3.5.6 or later, together with HSQLDB version 2.2.x or later. Otherwise, older
		...
		IAnsichtTab ansichtTab = this.setUpAnsichtAndTab( TestConstants.TABLENAME_TABDEF,
		...
		// here we have to query the hibernate-session-factory for objects. This way the factory does in implicit flush
		// to db before we use the simpleJdbcTemplate for querying new saved objects (e.g. sequence-values)
	...
	public void testFindAllWithSorting() throws OfdbMissingMappingException {
		...
		TabDef tabDef1 = DomainMockFactory.createTabDefMock( TABLENAME_TAB1, this.getTestBereich(),
				!isAppInitialized() );
		saveForTest( tabDef1 );
		TabDef tabDef2 = DomainMockFactory.createTabDefMock( TABLENAME_TAB2, this.getTestBereich(),
				!isAppInitialized() );
		saveForTest( tabDef2 );
	...
	public void testFindByCriteria() throws OfdbException {
		...
		this.setUpAnsichtAndTab( TestConstants.TABLENAME_BENUTZERBEREICH, "de.mw.mwdata.core.domain.BenutzerBereich",
				"benutzerBereich", BenutzerBereich.class );
		IAnsichtTab ansichtTab_TabDef = this.setUpAnsichtAndTab( TestConstants.TABLENAME_TABDEF,
				"de.mw.mwdata.core.ofdb.domain.TabDef", "tabDef", TabDef.class );
		...
		String sql = b.selectTable( "TabDef", "tDef" ).fromTable( "TabDef", "tDef" ).andWhereRestriction( "tDef",
				"name", OperatorEnum.Eq, TestConstants.TABLENAME_TABDEF, ValueType.STRING ).buildSQL();
		...
		// 2. test findByCriteria with enum-value (strategy: we fetch objects from hibernate-sessions, change them
	...
	public void testInitializeOfdbMapper() throws OfdbMissingMappingException {
		...
		IAnsichtTab viewTab = this.setUpAnsichtAndTab( TestConstants.TABLENAME_TABDEF,
				"de.mw.mwdata.core.ofdb.domain.TabDef", "tabDef", TabDef.class );
