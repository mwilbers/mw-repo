...
CREATE UNIQUE INDEX IDX_FX_ANSSP_TABKSPK ON FX_ANSICHTSPALTEN_K (TABAKEY, SPALTEAKEY, ANSICHTDEFID);
...
-- create new table BENUTZERANSICHT, BENUTZERANSICHTSPALTEN
CREATE TABLE BENUTZERANSICHT (
	DSID NUMBER(11,0) NOT NULL,
	ANSICHTDEFID NUMBER(11,0) NOT NULL,
	BENUTZERID NUMBER(11, 0) NOT NULL,
	BEZEICHNUNG VARCHAR(255), 
	angelegtAm TIMESTAMP NOT NULL,
	angelegtVon VARCHAR(255) NOT NULL,
	system NUMERIC(1) DEFAULT -1 NOT NULL,
	CONSTRAINT BENUTZERANSICHT_KEY_PK PRIMARY KEY (DSID)
);

ALTER TABLE BENUTZERANSICHT ADD FOREIGN KEY (ANSICHTDEFID) REFERENCES FX_ANSICHTDEF_K(DSID);

ALTER TABLE BENUTZERANSICHT ADD FOREIGN KEY (BENUTZERID) REFERENCES BENUTZERDEF(DSID);

CREATE UNIQUE INDEX IDX_BENANS_ANSDEFID ON BENUTZERANSICHT (ANSICHTDEFID, BEZEICHNUNG);

CREATE TABLE BENUTZERANSICHTSPALTEN (
	DSID NUMBER(11,0) NOT NULL,
	BENUTZERANSICHTID NUMBER(11,0) NOT NULL,
	ANSICHTSPALTENID NUMBER(11,0) NOT NULL,
	REIHENFOLGE NUMBER(11,0) NOT NULL,
	BREITE NUMBER(11,0) NOT NULL,
	angelegtAm TIMESTAMP NOT NULL,
	angelegtVon VARCHAR(255) NOT NULL,
	system NUMERIC(1) DEFAULT -1 NOT NULL,
	CONSTRAINT BEN_ANSICHTSPALTEN_KEY_PK PRIMARY KEY (DSID)
);

ALTER TABLE BENUTZERANSICHTSPALTEN ADD FOREIGN KEY (BENUTZERANSICHTID) REFERENCES BENUTZERANSICHT(DSID);
ALTER TABLE BENUTZERANSICHTSPALTEN ADD FOREIGN KEY (ANSICHTSPALTENID) REFERENCES FX_ANSICHTSPALTEN_K(DSID);

CREATE UNIQUE INDEX IDX_BEN_ANSICHTSPALTEN_ID ON BENUTZERANSICHTSPALTEN (BENUTZERANSICHTID, ANSICHTSPALTENID);

-- insert sequenzen für BENUTZERANSICHT, BENUTZERANSICHTSPALTEN
Insert into SYSSEQUENZ (SEQUENZID,NAME,LETZTEBELEGTENR,INKREMENT,GESPERRTVONLOGINID,SYSTEMDS,ANGELEGTAM,BESCHREIBUNG,ANGELEGTVON,SYSTEM) 
values ( (select max(nvl(s.sequenzid,0))+1 from SYSSEQUENZ s ) ,'BENUTZERANSICHT:DSID','0','1',null,'-1',to_date('29.11.18','DD.MM.RR'),null,'1','-1');

Insert into SYSSEQUENZ (SEQUENZID,NAME,LETZTEBELEGTENR,INKREMENT,GESPERRTVONLOGINID,SYSTEMDS,ANGELEGTAM,BESCHREIBUNG,ANGELEGTVON,SYSTEM) 
values ( (select max(nvl(s.sequenzid,0))+1 from SYSSEQUENZ s ) ,'BENUTZERANSICHTSPALTEN:DSID','0','1',null,'-1',to_date('29.11.18','DD.MM.RR'),null,'1','-1');

-- Benutzer Superadministrator anlegen
INSERT INTO BENUTZERDEF(DSID, BENUTZERNAME, PASSWORT, DEAKTIVIERT, ANGELEGTAM, ANGELEGTVON, SYSTEM) 
values( (select s.letztebelegtenr + s.inkrement from SysSequenz s where s.name = 'Benutzerdef:DSID'), 'Superadministrator', 'superadmin', 0, to_date('29.11.18','DD.MM.RR'), '1', '-1' );
