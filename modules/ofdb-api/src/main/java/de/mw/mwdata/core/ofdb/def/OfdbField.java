...
public class OfdbField {
	...
	/**
	 * itemKey is needed for unique Key-Identifier for storing in MapValue if
	 * e.g. there are two referenced tables in view with the same columnname
	 */
	private String itemKey;

	public OfdbField(final ITabSpeig tabSpeig, final IAnsichtSpalte ansichtSpalte, final CRUD crud) {
	...
	public boolean isEntity() {
		return (this.propValue instanceof IEntity);
	}

	public void setEditable(final boolean editable) {
		this.editable = editable;
		this.addDiagnose("editable", Boolean.valueOf(editable).toString());
	}
	...
	/**
	 *
	 * @return 0 if field belongs to the maintable of the view, otherwise > 0,
	 *         if field is an additional column joined as alias on sql
	 */
