...
public class MockFactory {
	...
	public OfdbValidatable createOfdbValidatorMock() {
		...
		EasyMock.expect(ofdbValidatorMock.isTableValid(EasyMock.isA(ITabDef.class))).andReturn(resultSet).anyTimes();
	...
	private static ViewConfigHandle createViewHandleMock() {
		...
		IAnsichtTab viewTable = DomainMockFactory.createAnsichtTabMock((AnsichtDef) viewDef, (TabDef) tabDef, 0, "x",
				null, null, null, true);
		IAnsichtSpalte viewProp = DomainMockFactory.createAnsichtSpalteMock((AnsichtDef) viewDef, tableProp, viewTable);
