...
/**
 * Adminuser-Filter that checks, if the logged user is Administrator. Else
...
public class AdminUserFilter extends UserFilter {
	...
	public void doFilter(final ServletRequest request, final ServletResponse response, final FilterChain chain)
		...
		// for development-purposes: get user Administrator and save it to session
		// (TODO)
		if (!SessionUtils.isUserLoggedIn(httpRequest.getSession())) {

			// FIXME: not yet implemented ... just new Benutzer() provisoric

			// Benutzer benutzerAdmin = ofdbRechteService.findBenutzerWithRechte(
			// de.mw.mwdata.core.Constants.USERNAME_ADMIN );
		...
		if (SessionUtils.isUserLoggedIn(httpRequest.getSession())) {
			...
			// 2. hier AdminUserFilter mit eigenem Filter überschreibne, der nur auswertet,
			// ob der an
